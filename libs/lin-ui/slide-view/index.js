...
Component({
  ...
  methods: {
    ...
    onTouchEnd(t) {
      ...
      this._length > s && (this.setData({ popup: !1, x: 0 }), this.onCloseTap()),
        (e > i && !1 === this.data.out) ||
          (i - e >= s
    ...
    onChange(t) {
      !this.data.out && t.detail.x < -this._threshold
        ? this.setData({ out: !0 })
        : this.data.out && t.detail.x >= -this._threshold && this.setData({ out: !1 });
    },
    onRightTap() {
      this.properties.autoClose && (this.setData({ popup: !1, x: 0 }), this.onCloseTap()),
        this.triggerEvent('lintap', 'click right', { bubbles: !0, composed: !0 });
    },
    onOpenTap() {
