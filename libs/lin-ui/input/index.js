...
Component({
  ...
  properties: {
    ...
    labelWidth: { type: Number, value: 200 },
    labelLayout: { type: String, value: 'left', options: ['left', 'right'] },
    disabled: Boolean,
    placeholderStyle: String,
    showEye: { type: Boolean, value: !1 },
  },
  data: {},
  attached() {},
  methods: {
    handleInputChange(e) {
      const { detail: t = {} } = e,
        { value: a = '' } = t;
      this.setData({ value: a }),
        eventBus.emit('lin-form-change-' + this.id, this.id),
        this.triggerEvent('lininput', e.detail);
    },
    handleInputFocus(e) {
      this.triggerEvent('linfocus', e.detail);
    },
    handleInputBlur(e) {
      this.validatorData({ [this.data.name]: e.detail.value }),
    ...
    handleInputConfirm(e) {
      const { detail: t = {} } = e,
        { value: a = '' } = t;
      this.setData({ value: a }), this.triggerEvent('linconfirm', e.detail);
    },
    onClearTap(e) {
      this.setData({ value: '' }), this.triggerEvent('linclear', e.detail);
    },
    getValues() {
